...
The other script is also relatively simple, it selects a random wallpaper from
my Wallpapers directory and passes it to ~pywal~ and ~betterlockscreen~.

#+BEGIN_SRC bash :tangle polybar/wallpaper-change :shebang #!/bin/bash
DIR=$HOME/Media/Wallpapers
image=$(find $DIR -type f | sort -R | head -n 1)
